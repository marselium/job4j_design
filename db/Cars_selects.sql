...
from car_bodies cb left join cars c on c.body_id = cb.id
where c.name is null order by cb.id

select ce.name as engine_name
from car_engines ce left join cars c on c.engine_id = ce.id
where c.name is null order by ce.id

select ct.name as transmission_name
from car_transmissions ct left join cars c on c.transmission_id = ct.id
where c.name is null order by ct.id